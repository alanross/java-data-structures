 ...
 *
 * Copyright 2009 Rednaxela This software is provided 'as-is', without any
 ...
 * documentation would be appreciated but is not required. 2. This notice may
...
public class KDTree2<T>
{
	...
	/**
	 * Construct a KDTree2 with a given number of dimensions and a limit on
	 * maxiumum size (after which it throws away old points)
	 */
	public KDTree2( int dimensions, Integer sizeLimit )
	{
	...
	private KDTree2( KDTree2<T> parent )
	{
	...
	public void addPoint( double[] location, T value )
	{
		...
		while( cursor.locations == null || cursor.locationCount >= cursor.locations.length )
		{
			if( cursor.locations != null )
			{
				...
				// Create child leaves
				KDTree2<T> left = new KDTreeChildNode( cursor );
				KDTree2<T> right = new KDTreeChildNode( cursor );
	...
	public List<T> nearestNeighbor( double[] location, int count, boolean sequentialSorting )
	{
		KDTree2<T> cursor = this;
		cursor.status = Status.NONE;
		double range = Double.POSITIVE_INFINITY;
		KDResultHeap resultHeap = new KDResultHeap( count );
		...
		ArrayList<T> results = new ArrayList<T>( resultHeap.values );
		if( sequentialSorting )
		{
			while( resultHeap.values > 0 )
			{
				resultHeap.removeLargest();
				results.add( ( T ) resultHeap.removedData );
			}
		}
		else
		{
			...
			{
				results.add( ( T ) resultHeap.data[ i ] );
			}
		}
	 ...
	 */
	private class KDTreeChildNode extends KDTree2<T>
	{
		private KDTreeChildNode( KDTree2<T> parent )
		{
			super( parent );
		}
	 ...
	 */
	private static class KDResultHeap
	{
